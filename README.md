...
<img src="studio/public/osvell.png" width="72" height="72" alt="Osvell icon" />
...
[**Download for Windows**](https://github.com/rjreny/osvell/releases/latest) · [Getting started](#make-it-yours) · [How Taste works](#meet-taste) · [Report an issue](https://github.com/rjreny/osvell/issues)

</div>

![Osvell Home, with a cinematic Interstellar backdrop and a shelf of recently watched films](docs/images/studio-home.jpg)

<p align="center"><sub>Osvell's actual interface, rendered in a browser with a sample library. Ratings and viewing activity are illustrative.</sub></p>

Your diary is more than a list of films. It's the directors you keep returning to, the unexpected five-star discoveries, and the comfort watches you've lost count of. Osvell brings that history together in a desktop app made for browsing—and turns it into a starting point for your next great watch.
...
- **Keep up with your film people.** Add friends by Letterboxd username and browse their recent public ratings inside Osvell.
...
- **Keep your library on your machine.** Osvell stores its library in a local SQLite database. Online services add metadata, diary updates, and optional AI analysis.
...
![Osvell Films, showing a searchable poster grid with sorting and decade filters](docs/images/studio-films.jpg)

### Your watching habits, at a glance

![A detail of Osvell Stats, showing watching activity, rating distribution, genres, and decades](docs/images/studio-stats.jpg)
...
The aim is a more personal, better explained shortlist. Osvell does not currently claim benchmark superiority over other recommendation services. [Read the technical overview →](docs/taste.md)

## Make it yours

1. **Install Osvell.** Open the [latest release](https://github.com/rjreny/osvell/releases/latest), download the Windows x64 `setup.exe`, and run it. Updates are available inside the app.
...
**Before you start:** Osvell currently ships for Windows x64. Taste uses your OpenRouter account; model, embedding, and optional web-search requests may incur provider charges. During analysis, film-history evidence and metadata are sent through OpenRouter to the selected services. Your library is stored locally, but online features require a connection and are not entirely on-device.

## Built with care, open to explore

Osvell uses **Tauri 2, Rust, React, TypeScript, and SQLite**. To run it from source, see the [developer README](studio/README.md). For the desktop host decision, see [why Tauri](docs/runtime-decision.md).

Found a rough edge or have an idea? [Open an issue](https://github.com/rjreny/osvell/issues). For a recommendation that misses, include what felt wrong about the connection—it's useful feedback for the next iteration.

Film metadata and artwork are provided by TMDB. This product uses the TMDB API but is not endorsed or certified by TMDB. Osvell is an independent project and is not affiliated with Letterboxd.
